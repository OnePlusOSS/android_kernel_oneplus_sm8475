...
#if ESE_COLD_RESET
// TODO-Keith Add cold_reset to nfc driver
...
#define UWB_TAG "OPLUS_UWB"
#define SR100_TAG "SR100T"

#define sr100_info(fmt, ...) \
    printk(KERN_INFO UWB_TAG " " SR100_TAG " " "%s: %d: " fmt "\n", __func__, __LINE__, ##__VA_ARGS__)

#define sr100_dbg(fmt, ...) \
    printk(KERN_DEBUG UWB_TAG " " SR100_TAG " " "%s: %d: " fmt "\n", __func__, __LINE__, ##__VA_ARGS__)

#define sr100_err(fmt, ...) \
    printk(KERN_ERR UWB_TAG " " SR100_TAG " " "%s: %d: " fmt "\n", __func__, __LINE__, ##__VA_ARGS__)
...
static void sr100_start_throughput_measurement(unsigned int type) {
  if (type == READ_THROUGH_PUT) {
    ...
    sr100_dbg("sr100_start_throughput_measurement: wrong type = %d",
           type);
  }
}
/******************************************************************************
...
static void sr100_stop_throughput_measurement(unsigned int type,
  ...
  if (type == READ_THROUGH_PUT) {
    ...
    sr100_dbg("sr100_stop_throughput_measurement: wrong type = %d",
           type);
  }
}
#endif
...
static int sr100_dev_open(struct inode* inode, struct file* filp) {
  ...
  sr100_dbg("Major No: %d, Minor No: %d", imajor(inode),
                iminor(inode));
...
static irqreturn_t sr100_dev_irq_handler(int irq, void* dev_id) {
  ...
  sr100_dbg("before wake up read_wq");
  wake_up(&sr100_dev->read_wq);
  sr100_dbg("after wake up read_wq");
  return IRQ_HANDLED;
}
  ...
  switch (cmd) {
    ...
    case SR100_SET_FWD:
      if (arg == 1) {
        ...
        sr100_dbg("FW download enabled.");
      } else if(arg == 0){
        is_fw_dwnld_enabled = false;
        sr100_dbg("FW download disabled.");
      }
      break;
    ...
    case SR100_GET_THROUGHPUT:
      if (arg == 0) {
        ...
        sr100_dbg("No of Write Bytes = %ld",
               sr100_through_put_t.total_through_put_wbytes);
        sr100_dbg("No of Read Bytes = %ld",
               sr100_through_put_t.total_through_put_rbytes);
        sr100_dbg("Total Write Time (uSec) = %ld",
               sr100_through_put_t.total_through_put_wtime);
        sr100_dbg("Total Read Time (uSec) = %ld",
               sr100_through_put_t.total_through_put_rtime);
        sr100_dbg("Total Write-Read Time (uSec) = %ld",
               sr100_through_put_t.total_through_put_wtime +
                   sr100_through_put_t.total_through_put_rtime);
    ...
    case SR100_ESE_RESET:
      sr100_dbg("SR100_ESE_RESET Enter");
      // TODO-Keith Add cold_reset to nfc driver
      /*
...
static int sr100_dev_transceive(struct sr100_dev* sr100_dev, int op_mode, int count ){
  ...
  switch(sr100_dev->mode){
    case SR100_WRITE_MODE:
    {
      ...
      if (ret < 0) {
        ret = -EIO;
        sr100_err("spi_write header : Failed.\n");
        goto transcive_end;
      } else {
      ...
      if(count > 0) {
        ...
        if (ret < 0) {
          ret = -EIO;
          sr100_err("spi_write payload : Failed.\n");
          goto transcive_end;
        }
      }
    ...
    case SR100_READ_MODE:
    {
      if(!gpio_get_value(sr100_dev->irq_gpio)){
        sr100_err("IRQ might have gone low due to write ");
      ...
      if (ret == 0) {
        sr100_err("wait_event_interruptible timeout() : Failed.\n");
      ...
      if(!gpio_get_value(sr100_dev->irq_gpio)){
        sr100_err("Spurious interrupt detected at second irq");
        retry_count++;
        if(retry_count == MAX_READ_RETRY_COUNT){
          retry_count = 0;
          sr100_err("Max retry count reached at second irq");
        } else{
          msleep(3);
          sr100_dev->irq_enabled = false;
      ...
      if(sr100_dev->totalBtyesToRead > MAX_UCI_PKT_SIZE) {
        sr100_err("Length %d  exceeds the max limit %d....",(int)sr100_dev->totalBtyesToRead,(int)MAX_UCI_PKT_SIZE);
        ret = -1;
        goto transcive_end;
      }
      if(sr100_dev->totalBtyesToRead > 0){
        ...
        if (ret < 0) {
          sr100_err("sr100_dev_read: spi read error.. %d\n ", ret);
          goto transcive_end;
        }
      }
      ...
      do{
        ...
        retry_count++;
        if(retry_count == 1000){
          sr100_err("Slave not released the IRQ even after 1ms");
          break;
        }
      }while(gpio_get_value(sr100_dev->irq_gpio));
      ret = spi_transcive_success;
...
static int sr100_hbci_write(struct sr100_dev* sr100_dev, int count ){
  ...
  if (ret < 0) {
    ret = -EIO;
    sr100_err("spi_write fw download : Failed.\n");
  ...
  return ret;
hbci_write_fail:
  sr100_err("sr100_hbci_write failed...%d", ret);
  return ret;
}
...
static ssize_t sr100_dev_write(struct file* filp, const char* buf, size_t count,
  ...
  if (count > SR100_MAX_TX_BUF_SIZE || count > SR100_TXBUF_SIZE) {
    sr100_dbg("Write Size Exceeds");
    ret = -ENOBUFS;
    goto write_end;
  }

  if (copy_from_user(sr100_dev->tx_buffer, buf, count)) {
    sr100_dbg("failed to copy from user space");
    return -EFAULT;
  }
  sr100_info("write count = %d", count);
#if (ENABLE_THROUGHPUT_MEASUREMENT == 1)
 ...
 ****************************************************************************/
 static ssize_t sr100_hbci_read(struct sr100_dev *sr100_dev,char* buf, size_t count){
  ...
  if(count > SR100_RXBUF_SIZE) {
    sr100_err("count(%d) out of range(0-%d)\n", count, SR100_RXBUF_SIZE);
    ret = -EINVAL;
    goto hbci_fail;
  }
  /* wait for inetrrupt upto 500ms after that timeout will happen and returns read fail */
  ret = wait_event_interruptible_timeout(sr100_dev->read_wq, sr100_dev->irq_received, sr100_dev->timeOutInMs);
  if (ret == 0) {
    sr100_info("hbci wait_event_interruptible timeout() : Failed.\n");
    ret = -1;
    goto hbci_fail;
  }
  ...
  if(!gpio_get_value(sr100_dev->irq_gpio)){
   sr100_info("IRQ is low during firmware download");
   goto hbci_fail;
  }
  ...
  if (copy_to_user(buf, sr100_dev->rx_buffer, count)) {
    sr100_info("sr100_dev_read: copy to user failed\n");
    ret = -EFAULT;
  }
  return ret;
hbci_fail:
  sr100_info("Error sr100_fw_download ret %d Exit\n", ret);
  return ret;
}
/******************************************************************************
...
first_irq_wait:
  ...
  if(!read_abort_requested) {
    ...
    if (ret) {
      sr100_dbg("wait_event_interruptible() : Failed.");
      goto read_end;
    }
  }
  if (read_abort_requested) {
  ...
  if(ret == spi_transcive_success){
    ...
    sr100_dbg(" irg is low due to write hence irq is requested again...");
    goto first_irq_wait;
  } else if(ret == spi_irq_wait_timeout){
    sr100_dbg("second irq is not received..Time out...");
    ret = -1;
  } else {
    sr100_dbg("spi read failed...%d", ret);
    ret = -1;
  }
read_end:
  retry_count = 0;
...
static int sr100_hw_setup(struct sr100_spi_platform_data* platform_data) {
  ...
  if (ret < 0) {
    sr100_info("gpio request failed gpio = 0x%x\n", platform_data->irq_gpio);
    goto fail;
  }
...
static int sr100_parse_dt(struct device* dev,
  ...
  if ((!gpio_is_valid(pdata->vbat_3v6_gpio)))
          return -EINVAL;
  sr100_info("sr100 : vdd_1v8_gpio = %d, vdd_1v8_rf_gpio = %d, vbat_3v6_gpio = %d \n",
  pdata->irq_gpio, pdata->vdd_1v8_rf_gpio,pdata->vbat_3v6_gpio);
#endif
  sr100_info("sr100 : irq_gpio = %d, ce_gpio = %d, spi_handshake_gpio = %d \n",
...
static int sr100_probe(struct spi_device* spi) {
  ...
  sr100_dbg("chip select : %d , bus number = %d",
                spi->chip_select, spi->master->bus_num);

  ret = sr100_parse_dt(&spi->dev, &platform_data1);
  if (ret) {
    sr100_err("%s - Failed to parse DT\n", __func__);
    goto err_exit;
  }
  ...
  if (sr100_dev == NULL) {
    sr100_err("failed to allocate memory for module data");
    ret = -ENOMEM;
    goto err_exit;
  }
  ret = sr100_hw_setup(platform_data);
  if (ret < 0) {
    sr100_err("Failed to sr100_enable_SR100_IRQ_ENABLE");
    goto err_exit0;
  }
  ...
  ret = spi_setup(spi);
  if (ret < 0) {
    sr100_err("failed to do spi_setup()");
    goto err_exit0;
  }
  ...
  if (ret < 0) {
    sr100_err("misc_register failed! %d", ret);
    goto err_exit0;
  }
  ...
  if (sr100_dev->spi->irq < 0) {
    sr100_err("gpio_to_irq request failed gpio = 0x%x",
                  platform_data->irq_gpio);
    goto err_exit1;
  }
