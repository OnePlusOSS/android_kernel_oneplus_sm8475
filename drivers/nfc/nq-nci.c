				...
				size_t count, loff_t *offset)
{
	...
	if (cmdcmp(tmp, on_unlocked, sizeof(on_unlocked)) == 0
		...
		if (!wakeup_cmd) {
		    dev_err(&nqx_dev->client->dev, "%s: kzalloc wakeup_cmd failed\n",
		        __func__);
		    ret = -ENOMEM;
		    goto out_free;
		}
		...
		ret = i2c_master_send(nqx_dev->client, wakeup_cmd, NCI_GET_FW_CMD_LEN);
		...
		if (ret < 0) {
		    ...
		    dev_err(&nqx_dev->client->dev,
		        "%s: Enter. i2c_master_send wakeup_cmd\n", __func__);
		}
	}
	#endif
